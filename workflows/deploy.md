...
7. **Escape `$` to `$$` in `env_file` values.** Compose v2 interpolates `$` in `env_file` values, which silently mangles bcrypt hashes (every `$2b$10$abc…` ends up missing the segment after each `$` because `$abc…` is treated as an unset variable). Symptom: admin login returns "Invalid email or password" with no server-side error. Fix: write the hash as `$$2b$$10$$abc…`. `tools/bootstrap.sh` does this automatically; `server/.env.production.example` documents it for the manual path.
