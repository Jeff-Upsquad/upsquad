...
import { downloadPosterPng, posterFilename, wait } from '../dream-team/downloadPoster'

function Poster({ concept, format, innerRef }) {
  if (concept === 'split') return <SplitPoster format={format} innerRef={innerRef} />
  if (concept === 'lineup') return <LineupPoster format={format} innerRef={innerRef} />
  return <RosterPoster format={format} innerRef={innerRef} />
}

export default function PosterStudio() {
  const [conceptId, setConceptId] = useState('roster')
  ...
  const captureCurrent = useCallback(async () => {
    ...
    await downloadPosterPng(node, posterFilename(conceptId, currentFormat, 'upsquad-content-squad'), currentFormat)
  }, [conceptId])
      ...
      <div>
        ...
        {format.id === 'stories' && (
          <p className="mt-1.5 text-xs text-text-secondary leading-relaxed">
            Reels/Stories safe zone: headline and roles sit in the middle of the frame so the username, caption, and tab bar cannot cover them.
          </p>
        )}
      </div>
