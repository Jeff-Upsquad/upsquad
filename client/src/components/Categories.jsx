...
function Drawer({ squad, onClose }) {
  ...
  return (
    <>
      {/* Backdrop — full screen including navbar */}
      <div
      ...
      {/* Panel */}
      <div className="fixed right-3 top-3 bottom-3 w-[calc(100%-1.5rem)] sm:w-[480px] bg-white z-[61] shadow-2xl rounded-2xl flex flex-col overflow-hidden animate-slide-in">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-6">
          ...
          <div className="flex flex-col gap-2.5">
            {d.talents.map((talent) => (
              <div
                key={talent.name}
                className="bg-gray-50 border border-gray-100 rounded-xl p-4 flex items-start gap-4"
              >
                <span className="text-3xl mt-0.5 shrink-0">{talent.emoji}</span>
                <div>
                  <p className="text-sm font-semibold text-slate-800 leading-snug">{talent.name}</p>
                  <p className="text-xs text-gray-400 mt-1 leading-relaxed">{talent.desc}</p>
                </div>
              </div>
            ))}
          </div>
